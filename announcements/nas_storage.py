    ...
    def _save(self, name, content):
        ...
        try:
            ...
            persist_app_nas_file(
                tmp_path=tmp_path,
                mount_dest=dest,
                folder_rel_base=announcement_nas_rel_base(),
                file_rel=name,
            )
            return name
        finally:
...
def _dsm_download_to_cache(rel_name: str) -> Path | None:
    ...
    full_rel = f'{announcement_nas_rel_base()}/{rel_name.lstrip("/")}'
    try:
...
def ensure_announcement_nas_dir() -> Path:
    ...
    try:
        ...
        persist_app_nas_mkdir(announcement_nas_rel_base())
    return root
...
def _announcement_rclone_target(rel_name: str) -> str:
    ...
    return app_storage_rclone_target(announcement_nas_rel_base(), rel_name.lstrip('/'))
...
def _rclone_upload_file(local_path: Path, rel_name: str) -> None:
    ...
    persist_app_nas_file(
        tmp_path=local_path,
        mount_dest=announcement_nas_abs_root() / rel_name.lstrip('/'),
        folder_rel_base=announcement_nas_rel_base(),
