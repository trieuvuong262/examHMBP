"""Seed menu Phiếu nhập cho Kho sản phẩm — kế thừa quyền từ Tồn kho / Danh mục."""
...
def seed_forward(apps, schema_editor):
    ...
    for group in PermissionGroup.objects.all():
        ...
        if 'receipts' in menus:
            continue
        source = menus.get('stock') or menus.get('products') or ksp
        flags = _flags(source)
        if not _has_any(flags):
            continue
        menus['receipts'] = flags
        ksp['menus'] = menus
...
def seed_backward(apps, schema_editor):
    ...
    for group in PermissionGroup.objects.all():
        ...
        if not isinstance(ksp, dict):
            continue
        menus = dict(ksp.get('menus') or {}) if isinstance(ksp.get('menus'), dict) else {}
        if 'receipts' not in menus:
            continue
        menus.pop('receipts', None)
        ksp['menus'] = menus
        perms['kho_san_pham'] = ksp
        group.module_permissions = perms
        group.save(update_fields=['module_permissions'])
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('hrm', '0099_seed_san_xuat_plan_inter_step_menu'),
    ]
