...
MODULE_SUBMENUS: dict[str, list[dict]] = {
    ...
    MODULE_SAN_XUAT: [
        ...
        {'key': 'bom', 'label': 'BOM (đã gộp Hồ sơ thiết kế)', 'icon': 'bi-diagram-3'},
        {'key': 'products_nvl', 'label': 'Sản phẩm – NPL (đã ẩn khỏi sidebar)', 'icon': 'bi-box'},
        {'key': 'capacity_load', 'label': 'Tải theo tổ (đã gộp Năng lực SX)', 'icon': 'bi-bar-chart-steps'},
        ...
        {'key': 'plan_overall', 'label': 'Kế hoạch tổng thể (đã gộp)', 'icon': 'bi-calendar-range'},
        {'key': 'plan_detail', 'label': 'Kế hoạch chi tiết (đã gộp)', 'icon': 'bi-list-columns'},
