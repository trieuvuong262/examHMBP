...
def _export_plan_npl(request):
    ...
    return _hub_model_export(
        ...
        row_fn=lambda p: {
            'Mã': p.code,
            'Tên': p.name,
            'Đơn KHSX': getattr(p.sales_order, 'code', '') if p.sales_order_id else '',
            'KH tổng thể': getattr(p.overall_plan, 'code', '') if p.overall_plan_id else '',
            'Trạng thái': _status(p),
        },
    )
...
def _export_npl_pr(request):
    ...
    return _hub_model_export(
        ...
        select_related=('material_plan', 'sales_order'),
        row_fn=lambda r: {
            'Mã': r.code,
            'Đơn KHSX': getattr(r.sales_order, 'code', '') if r.sales_order_id else '',
